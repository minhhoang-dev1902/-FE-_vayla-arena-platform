...
export default function SubmissionDetailPage() {
	...
	// Fetch all submissions and find matching one
	const { data: resMySubmissions } = useGetMySubmissions(
		new MySubmissionsSearchClass({ status: TrackStatusEnum.NONE }),
	);
	const submissions = resMySubmissions?.data?.submissions ?? [];
	const submission = submissions.find(s => s.submissionId === id);
	...
	const eventTracks = (resEventTracks?.data?.tracks ?? [])
		...
		return (
			<main className="min-h-dvh bg-white">
				...
				<div className="flex min-h-[60vh] items-center justify-center">
					<p className="text-[14px] text-dark-sub-primary">Loading...</p>
				</div>
			</main>
		);
	}
	...
	return (
		<main className="bg-[#ffffff] pb-10 container">
			...
			<div className="mx-auto w-full max-w-[480px] px-4">
				{/* ── Stats row ── */}
				<div className="mt-[30px] grid grid-cols-3 divide-x divide-[#F0F0F0] gap-3">
					<StatItem
				...
				{eventTracks.length > 0 && (
					<section className="mt-6">
						<div className="mb-4 flex items-center justify-between">
							<h2 className="text-[16px] font-bold leading-[22px] text-dark-primary">
								More from Challenge
							</h2>
							<Button
								variant="ghost"
								onClick={() => router.push(`${NAVIGATE.DISCOVERY}/${submission.eventId}`)}
								className="h-auto p-0 text-[12px] font-bold uppercase tracking-[0.12em] text-secondary-button"
							>
								View All
							</Button>
						</div>
						...
						<div className="flex flex-col gap-3">
							{eventTracks.map(track => (
								<div key={track.submissionId} className="flex items-center gap-3">
									<CustomImage
										...
										width={44}
										height={44}
										className="size-[44px] rounded-[10px] object-cover"
									/>
									<div className="min-w-0 flex-1">
										<p className="truncate text-[14px] font-bold leading-[18px] text-dark-primary">
											{track.trackTitle}
										</p>
										<p className="truncate text-[11px] leading-[16px] text-dark-sub-primary">
											BY {track.artistName.toUpperCase()}
										</p>
									</div>
									<div className="shrink-0 text-right">
										<p className="text-[14px] font-bold leading-[18px] text-dark-primary">
											{new Intl.NumberFormat("en-US").format(track.voteCount)}
										</p>
										<p className="text-[10px] uppercase tracking-[0.1em] text-dark-sub-primary">
											Votes
										</p>
									</div>
								</div>
